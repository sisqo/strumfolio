...
/** What the arrows now carry: `song` above names every fixture after its own slug. */
function step(slug: string) {
...
describe('seriesOf', () => {
  it('finds the previous and next song in the same songbook, in list order', () => {
    ...
    assert.deepEqual(seriesOf(songs[1], songs), {
      position: 2,
      total: 3,
      previous: step('a'),
      next: step('c'),
  ...
  it('has no previous at the start and no next at the end', () => {
    ...
    assert.deepEqual(seriesOf(songs[0], songs), {
      position: 1,
      total: 2,
      previous: null,
      next: step('b'),
...
describe('siblingsOf', () => {
  it('gives the songbook in list order, and nothing from any other', () => {
    ...
    assert.deepEqual(siblingsOf(songs[0], songs), [step('a'), step('b')])
  })
})

describe('favoritesSeries', () => {
  const siblings = ['a', 'b', 'c', 'd', 'e'].map(step)
  ...
  it('steps between starred songs only, keeping the songbook\'s own order', () => {
    ...
    assert.deepEqual(resolved, { position: 2, total: 3, previous: step('b'), next: step('e') })
  })

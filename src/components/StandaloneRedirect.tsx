 ...
 * `/?app=1` with a branch in the middleware, and it is deliberately not taken: it would put a
 * new parameter on the one URL the service worker precaches, so it would also need a line in
 * `ignoreURLParametersMatching` (`sw.ts`), and getting that wrong means the installed app
 * misses its precache and opens to nothing offline. Rejected in favour of one `useEffect`
 * whose worst failure is a flash of the wrong page. It also covers something the manifest
 * could not: iOS freezes the manifest at install time, so every iPhone already carrying this
 * icon would keep `start_url: '/'` whatever a new deploy says.
 *
